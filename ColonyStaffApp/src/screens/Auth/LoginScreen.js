...
const LoginScreen = ({ navigation }) => {
 ...
 const handleLogin = async () => {
  try {
    ...
    Alert.alert("Login Failed", error.response?.data?.message || "Check your ID and password");
  } finally {
    setLoading(false);
  }
};
...
const styles = StyleSheet.create({
  container: { flex: 1, padding: 25, backgroundColor: Colors.white, justifyContent: 'center' },
  title: { fontSize: 26, fontWeight: 'bold', color: Colors.primary, marginBottom: 40, textAlign: 'center' },
